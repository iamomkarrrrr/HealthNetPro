...
const SAMPLE_METRICS = '{"activeOutbreaks": 5, "containedOutbreaks": 3, "closedOutbreaks": 12}'
...
const ReportsPage = () => {
  ...
  const [scopeFilter, setScopeFilter] = useState('OUTBREAK')
  const [dateFilter, setDateFilter] = useState('')
  const [fetchError, setFetchError] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ scope: 'OUTBREAK', metrics: SAMPLE_METRICS, generatedDate: new Date().toISOString().slice(0, 10) })
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setForm({ scope: 'OUTBREAK', metrics: SAMPLE_METRICS, generatedDate: new Date().toISOString().slice(0, 10) })
      setShowForm(false)
      fetchReports()
    } catch (err) {
    ...
    <DashboardLayout>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="row g-3">
              ...
              <div className="col-12">
                ...
                <textarea name="metrics" className="form-control font-monospace" rows={3} value={form.metrics} onChange={handleChange} placeholder={SAMPLE_METRICS} />
              </div>
            </div>
      ...
      {loading ? <Loader message="Loading reports…" /> : reports.length === 0 ? (
        ...
        <div className="row g-4">
          {reports.map(r => {
            const parsed = tryParseJson(r.metrics)
            return (
