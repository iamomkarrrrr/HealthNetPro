...
const CaseUpdatesPage = () => {
  ...
  // Load disease cases for the selector
  useEffect(() => {
    getAllDiseaseCases()
      .then(res => setCases(res.data?.data ?? []))
      .catch(err => setFetchError(err?.response?.data?.message || 'Failed to load cases'))
      .finally(() => setLoadingCases(false))
  }, [])

  // Fetch updates whenever selected case changes
  const fetchUpdates = useCallback(() => {
    ...
    getCaseUpdatesByCaseId(selectedCaseId)
      ...
      .catch(err => {
        setFetchError(err?.response?.data?.message || 'Failed to load case updates')
        setUpdates([])
      })
  ...
  const openEdit = (u) => {
    setEditTarget(u)
    setForm({ notes: u.notes, date: u.date, status: u.status })
    setSaveError('')
    setSaveSuccess('')
    setShowForm(true)
  }
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editTarget) {
        await updateCaseUpdate(editTarget.id, {
          notes: form.notes,
          date: form.date,
          status: form.status,
        })
        setSaveSuccess('Case update saved successfully.')
      } else {
        ...
        setSaveSuccess('Case update created successfully.')
      }
      setForm(EMPTY_FORM)
      setShowForm(false)
      setEditTarget(null)
  ...
  return (
    <DashboardLayout>
            ...
            {loadingCases ? <Loader message="Loading cases…" /> : (
              cases.length > 0 ? (
                <select
                  ...
                  onChange={e => { setSelectedCaseId(e.target.value); setShowForm(false) }}
                >
                  <option value="">— Select a case —</option>
                  {cases.map(c => (
                    <option key={c.id} value={c.id}>
                      #{c.id} — {c.diseaseType} (Citizen #{c.citizenId}) [{c.status}]
                    </option>
                  ))}
                </select>
              ) : (
            ...
            <div className="row g-3">
              <div className="col-12">
                ...
                <textarea
                  ...
                  placeholder="e.g. Patient fever reduced, continue hydration and medication"
                />
              </div>
              <div className="col-md-6">
      ...
      {selectedCaseId && (
        <Card
          title={`Updates for Case #${selectedCaseId}`}
          header={
              ...
              {updates.map(u => (
                <li key={u.id} className="py-3 border-bottom">
                  <div className="d-flex justify-content-between align-items-start">
                    ...
                    <button className="btn btn-sm btn-outline-secondary flex-shrink-0" onClick={() => openEdit(u)}>
                      Edit
                    </button>
                  </div>
                </li>
